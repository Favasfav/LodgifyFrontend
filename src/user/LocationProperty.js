...
function LocationProperty() {
  ...
  const [property_type,setpropert_type]=useState([]);
...
const [propertytype,setpropertytype]=useState([]);
    ...
    const fetchPropertyList = async () => {
      try {
        const response = await Axiosinstance.get(
          `partner/getpropertybylocation/${maplocation}`
        );
        if (response.data) {
          setProperties(response.data);
        } else {
          setError("No data available."); // You can customize the error message
        }
      } catch (error) {
        setError("Error fetching data. Please try again later."); // You can customize the error message
      } finally {
        setLoading(false);
      }
    };
  ...
  onChange={(e) => {
    ...
    console.log("Updated propertytype:", propertytype); // Add this line
  }}
/>
